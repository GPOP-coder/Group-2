...
| `PCA 3145 Attendant.` | La Luz (pool bar/outlet) attendant — no current labor standard exists for this job; department confirmed they still use "attendant" as a real role, not fully retired | **Fixed** — added the job under La Luz, mapped as a Casual Dining—type role (only "Casual Dining" options were available in the master-job list: bar back, bartender, bus person, cook, host, runner). No labor standard attached (deliberately, since one didn't exist before). |
