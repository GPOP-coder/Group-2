...
- **Portfolio-wide bartender/banquet-labor KBI gap (confirmed 9/10/26 weekly call):** the same underlying gap shows up at three different properties independently — Hilton Scottsdale (John's granular-cover-types feedback), Westin La Paloma (bartender/captain banquet counts are a manual input KBI, no interfaceable field exists — BEO notes are free text, not structured data), and Marriott Del Mar (no bartender job/standard exists at all; bar schedule still occupancy-based with zero beverage revenue flowing). This is very likely one methodology question across the whole portfolio, not three unrelated property issues — worth raising that way with Alain/Taylor rather than letting each property fix it in isolation. See [2026-09-10_weekly-hma-call-notes.md](2026-09-10_weekly-hma-call-notes.md) for full detail.
