...
- [x] **Sandra's session held, 9/11/26, ~2:30–2:37 PM PT start.** Rushed, scope kept expanding beyond "just what she needs for now." Full detail: [Sandra Academia — Ad-Hoc Training](2026-09-11_sandra-academia-adhoc-training.md). Two schedule anomalies found; one (Laundry placeholder back-to-back AM/PM shift) resolved 9/12/26, see below. Housekeeping numbers (6% over last week, 25% underscheduled next week) independently reconfirmed matching Thursday's session, and confirmation Sandra forecasts for both Marriott Del Mar and DoubleTree San Diego Del Mar. **Follow-up locked: Tuesday 9/15/26, 10 AM–12 PM PT, Teams** — before that week's labor meeting.
- [x] **Laundry placeholder back-to-back AM/PM shift anomaly resolved, 9/12/26.** Root cause: the placeholder had no Employee Maintenance availability restrictions, so autoschedule drafted it into consecutive AM and PM shifts. Pete corrected the availability settings. Broader lesson logged at [pws/foundations/03-implementation-analysis-methodology.md § 5c](../../../foundations/03-implementation-analysis-methodology.md) — placeholders need real availability restrictions, not just Sub Only + schedule-export exclusion. Worth auditing other existing placeholders for the same gap.
- [x] **Arterra Bartender confirmed working, update 9/12/26** — beverage-revenue-based (Bar Revenue KBI), actuals flowing correctly for the week of 9/5–9/11/26. Dropped from the client-facing status report per Pete (it works, no need to draw attention to it) — kept here for the internal record.
- [ ] **Banquet Bartender — real gap, framing corrected twice, 9/12/26.** First pass wrongly borrowed Arterra's occupancy-vs-beverage-revenue framing; that never applied. Second pass overcorrected the other way, calling it a non-issue. **Per Pete: the property genuinely runs banquet bars staffed by real banquet bartenders — there's just no job or labor standard in Paychex or Unifocus to represent that work.** Two related KBIs exist — BQT # Bartenders Needed and BQT # Stations Attendants Needed — but both require manual forecast/actuals entry (unlike the rest of the banquet forecast, which imports from CI) and neither is tied to a labor standard, so entering them doesn't generate shifts. **Recommended fix, confirmed still valid by Pete:** build an independent BQT Bartender job driven off the existing Reception KBIs.
- [ ] **"Eric's" fully-blacked-out availability** — the second schedule anomaly from the 9/11 session, still unresolved.
...
- [x] CI/banquet mapping accuracy — 1:30pm 9/9/26 Stephanie Simmons session held. Confirmed Unifocus reads the granular "Group Type" CI field (~10 options, where AA and other mystery codes live) rather than the coarser "Business Type" field (3-4 options). **Reframed 9/12/26, per Pete: this isn't "Unifocus reads the wrong field" — the coarser field is likely the one that should have been used; pulling the granular field was probably the wrong original call, likely true portfolio-wide (not Del Mar-specific), worth raising with Nicole Mendez/Devon Peters, but not an urgent Del Mar fix** (remapping the whole portfolio is real effort, paying off mainly if HM Alpha adds more properties). Also surfaced: no Banquet Bartender job/standard exists, though the work is real (see 9/12/26 note above), and Actual KBI entry ownership is undecided. Separately, a same-day Teams thread with Taylor Walton made progress on the interface/booking-type side — Group, Local, and Rooms Only mapped, Rooms Only confirmed → Group. Full detail: [Stephanie Simmons CI Integration Meeting, 9/9/26](2026-09-09_stephanie-simmons-ci-integration-meeting.md), [Banquet/CI Mapping Teams Thread, 9/9/26](2026-09-09_taylor-walton-banquet-mapping-teams.md)
- [ ] Map every remaining Group Type value and Group Type × Event Type combination (Group, Local, Rooms Only done) — the real near-term work, since Group Type is the field actually in use. The unexplained "AA" code is likely just one of the remaining unmapped values.
- [ ] Raise the Business Type vs. Group Type architectural question with Nicole Mendez and Devon Peters — a strategic/portfolio-wide conversation, not a Del Mar action item.
...
- [ ] **Arterra -21% whole-department variance** (F&B outlet, corrected from an earlier "Arteta" mishearing), flagged live 9/10/26 by Mark Ziomek during the Weekly Labor Summary walkthrough — individual job variances were modest (Bartender -1%, Buser -17%, Supervisor -3%), department total was not; not yet investigated. Distinct from the resolved Arterra Bartender revenue-standard finding above.
