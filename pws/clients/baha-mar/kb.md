...
| RMSOPS-12810 | Baha Mar Schedule Export — Indicate an OFF day for Unscheduled Days (Jira, RMS Operations project — not a UNIFOCUS-prefixed support ticket) | 🟡 Pending Development / Pending Client, per Jira 9/11/26. Monali Desai attached `schedule.csv` and directly mentioned Pete for review (2:35 PM GMT, 9/11/26) — full ask only visible in Jira itself, not captured in the email notification. Pete checking with Ahmed Chadid first on whether this is his to run with. **Pete's angle: wants to use this as a legitimate reason to connect directly with the property and arrange a visit** — ties to the broader Baha Mar Casino/direct-relationship opportunity in Properties Overview above, and to [[project_unifocus-post-ralph-relationships]] (building direct relationships independent of the Unifocus-side chain of command). |
