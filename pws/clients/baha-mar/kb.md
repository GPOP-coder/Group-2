...
| Baha Mar Casino | — | Potential future direct PWS client — **also a live referral opportunity under Pete's Unifocus Referral Agreement (contacts: Clay Price, Ed Peek).** Pete was denied a referral commission on the original Grand Hyatt Baha Mar deal (closed after he'd left Unifocus) and had to return later to fix the resulting implementation gaps. Now weighing pursuing Casino himself, this time ensuring he gets both the referral commission AND the implementation work — he's the only one left who knows how to build casino KBIs/labor standards. **9/2/26: followed up with Clay Price, awaiting reply. Fallback if Unifocus doesn't move: Pete pursues it directly.** Non-compete carve-out already covers this — CTF BM Operations d/b/a Baha Mar (the named carve-out) is the owning entity that would decide to add casino to Unifocus, and it owns Rosewood/SLS/GH plus the CTF Operations property record itself. Pete already has separate direct agreements with Rosewood and Grand Hyatt (CTF-approved) and is onboarded as a vendor with both. Full context: `pws/foundations/07-unifocus-contract-rewrite-2026.md`, `pws/clients/baha-mar/2026-09-02_casino-prospecting-followup-email.md`. |
...
**No Infor access — campus-wide (noted 8/25/26).** Neither Pete nor (as far as known) Unifocus support has direct access to Infor, the property's actual-hours source system. Any variance/reconciliation work comparing Unifocus dashboard figures to Infor actuals (e.g. UNIFOCUS-260831) depends on the property supplying the Infor-side numbers — can't be pulled independently.
...
| Nicola Murgia | — (role TBD) | Baha Mar (campus-level) | nicola.murgia@bahamar.com \| +1 242 359 1042. Existing contact from prior scheduling/rooms-forecast threads (Nov 2025); identity confirmed 9/19/26 via email search ahead of the planned 9/23/26 Bahamas cruise-stop visit. |
...
| UNIFOCUS-259575 | Rosewood Baha Mar — Unable to upload KBI | ✅ Resolved — filed by Emre 8/14/26 10:30 AM, marked urgent (risk was blocking his Monday 8/17/26 2027 payroll budget start). Emre confirmed the fix; Nguon Zhong closed the ticket 8/21/26, 10:12 PM. |
| UNIFOCUS-260831 | Rosewood Baha Mar — Errors in Labor Dashboard (hours variance vs. Infor) | 🔴 Open — filed 8/25/26 9:59 AM. Property reports Labor Dashboard showing significantly more hours than possible for the entire hotel, vs. actual hours from Infor for July. **Infor expected actual worked hours: 86,688.15** (per property). **Unifocus Metrics and Reports > Actual vs Forecast (Rolling AI) dashboard: 151,451** for the same period — the implausible/inflated figure driving the ticket. **Unifocus Labor Effectiveness report** (Rosewood Baha Mar, 7/1–7/31/26), Worked Hours summed across Rooms + F&B + Other Operating + Non Operating (Salaries excluded — matches Aaron's dashboard scope, which also excludes Salaries): **70,131.76**. Three different totals now, none matching: Infor 86,688.15 / dashboard 151,451 / Labor Effectiveness report 70,131.76 — the Labor Effectiveness figure being lower than Infor (rather than the dashboard's inflation) suggests either a scope/division mismatch or two distinct problems, not one. **No Infor access, campus-wide** — Infor-side numbers depend on the property supplying them. Not yet diagnosed. |
| RMSOPS-12810 | Baha Mar Schedule Export — Indicate an OFF day for Unscheduled Days (Jira, RMS Operations project — not a UNIFOCUS-prefixed support ticket) | 🟡 Pending Development / Pending Client, per Jira 9/11/26. Monali Desai attached `schedule.csv` and directly mentioned Pete for review (2:35 PM GMT, 9/11/26). **Full LOE description obtained 9/19/26:** current UF→Infor schedule export only includes days with an active shift; Infor can't clear a stale day on its own, so moving a shift (e.g. Fri→Sat) leaves the employee scheduled 6 days in Infor instead of 5, which then throws a no-show exception and delays payroll. Client wants the export to include all 7 days always, coding unscheduled days (and the day after a midnight-crossing shift) as "OFF," 00:00–00:00 — exact code/format TBD between property and Unifocus interface team. **Pete's angle: wants to use this as a legitimate reason to connect directly with the property and arrange a visit** — ties to the broader Baha Mar Casino/direct-relationship opportunity in Properties Overview above, and to [[project_unifocus-post-ralph-relationships]] (building direct relationships independent of the Unifocus-side chain of command). **Materializing 9/23/26:** Pete's Disney Cruise stops in Nassau that day — using the port day to visit Baha Mar, mention this ticket in person, and meet with Valquir Correa (confirmed), hopefully Nicola (identity TBD) and Emre Kenan. See `2026-09-23_bahamas-cruise-stop-visit-plan.md`. |
