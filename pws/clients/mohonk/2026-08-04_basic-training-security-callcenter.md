...
**Sub Only feature taught live — using Carol Carroll's F&B secondary jobs as the example.** Same employee maintenance screen also showed Carol's *other* secondary jobs — Buffet Attendant and Server Assistant in the Main Dining Room (owned by James/F&B, her home department for those roles) — both already correctly set to **Sub Only: Yes**. James had set this specifically because Carol was getting auto-scheduled into buffet/main-dining shifts she was only meant to cover as backup. Pete explained the mechanic to Ken/Molly: Sub Only marks a job "substitute only" — the employee stays eligible to be manually assigned but the auto-scheduler will never pull them into it on its own. Generalized takeaway for Security: any Security officer holding a secondary job elsewhere (e.g., banquets) that shouldn't auto-fill every week should get Sub Only set to Yes on that job — same fix pattern as James already applied for Carol's F&B side, just not yet applied on the Security side (see the separate secondary-job-gap finding above, which is a different problem — that job is missing from Unifocus entirely rather than present-but-unconstrained).
