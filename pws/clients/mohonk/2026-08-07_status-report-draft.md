...
- **Front Desk** — one overnight employee's actual hours aren't flowing into Unifocus reporting despite clocking in correctly and being paid correctly in ADP. A specific new diagnostic lead (an ADP Pay Code discrepancy) was identified this week. Stacy Zappone (Front Office/Guest Services) is checking ADP directly and is expected to provide details of our findings.
- **Culinary Extern hours** are not coding to the correct ADP jobs — identified this week, resolution path (direct job-coding vs. op-code bundling) to be worked out on a future visit.
...
- Two fix directions remain on the table, not yet decided: compress the forecast-to-generation sequence earlier in the week, or move the Wednesday labor meeting to Thursday. Aim for a property-wide schedule publish by Thursday EOD.
...
- Build labor standards for banquet culinary workload — none currently exist.
- Resolve Culinary Extern ADP job-coding.
