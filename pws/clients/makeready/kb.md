<!-- © Peter A. Castellano — Proprietary. Not for distribution or model training. -->

# MakeReady — Client Knowledge Base

## Overview

**MakeReady** (part of Rockbridge Capital) is a hotel management company operating a portfolio of independent boutique properties. Pete Castellano is engaged as a Unifocus trainer for their implementation.

- **Corporate Contact:** Michelle Lewis (Corp. Dir. Rooms & Guest Experience) | mlewis@makereadyexperience.com | 510-827-2000 | 1202 Dragon St. STE 104, Dallas TX 75207
- **Configuration Owner:** Steve Carrell (Unifocus)
- **Trainer:** Pete Castellano (PWS)
- **Post-config Owner:** John Grech (Unifocus)
- **Post go-live Owner:** Taylor Walton (Unifocus)

---

## Active Properties

### The Moran CITYCENTRE, Houston

- **Full name:** The Moran CITYCENTRE
- **Brand:** Independent boutique (AAA Four Diamond) — NOT a major chain flag
- **EMS:** Unknown — to be confirmed
- **Location:** CITYCENTRE district, Houston TX (~15 mi from downtown)
- **Property details:**
  - 244 rooms and suites; floor-to-ceiling windows
  - Recently completed multi-million-dollar renovation
  - F&B: Café Moran (breakfast) + The Allegory Bar (950 sq ft terrace)
  - Outdoor pool, complimentary parking and car service (5-mi radius)
  - Full meeting/event space
- **Timeline:** Pete onsite training Aug 10–14, 2026
- **Open item:** Confirm EMS system before config/pre-training review

---

### Heathman Hotel, Portland

- **Brand:** Independent — confirm flag
- **EMS:** Unknown — to be confirmed
- **Location:** Portland, OR
- **Timeline:** Dates TBD; Ralph Varble confirming
- **Open item:** Confirm brand flag and EMS

---

## Notes

- MakeReady properties are independent boutique hotels — do not assume a branded EMS (CI/TY, Envision, Delphi) without confirming
- Steve Carrell completing config before Pete arrives; pre-training standards review required (Ralph directive, June 24, 2026)
- **The Joseph Nashville (Luxury Collection/Marriott) is NOT a MakeReady property** — it is managed by Highgate; Steve Carrell did that consulting engagement; Pete has no involvement

---

## Related Files

- [HM Alpha Client KB](../hmalpha/kb.md)
