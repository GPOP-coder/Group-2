...
Loads at the start of every session. Keep current. Last updated: 2026-09-09
...
| Richard ("Uncle Dick") Castellano | Pete's paternal uncle | Lives in a San Diego-area community with active social life (weekly karaoke, bocce, wineries); has a girlfriend (name TBD). **Correction 9/17/26:** the daughter who died of breast cancer at 34 is his girlfriend's daughter, not Dick's biological daughter (not blood-related to Pete) — diagnosed at 23 during pregnancy with a delayed diagnosis; possible xylene exposure during pregnancy raised as a concern. Previously untracked connection until Pete's 9/7–9/12/26 San Diego trip, when Dick drove down and had dinner with Pete 9/11/26 — see `journal/2026-09-11_uncle-dick-dinner-san-diego.md`. |
...
| pcastellano@unifocus.com | Unifocus work/contractor account — M365 connector approved and active. **Identity dates back to 2009.** Around Pete's early return-as-contractor trips (e.g. the Nov 2025 Baha Mar visits), this identity wasn't yet fully restored — those trip confirmations went to his personal email instead. **Full pcastellano@unifocus.com identity was restored during one of those first trips.** **Data-quality flag (9/19/26): the Nov 2025 Miramar Transportation confirmation cc'd "peter.castellano@gmail.com" — not a real address on file; Pete's actual Gmail is `pete.castellano@gmail.com`.** Some early Baha Mar-side records may also have used `peter.a.castellano@outlook.com` and possibly `pete@platinumworkforcestrategies.com` — likely artifacts of the personal/Unifocus identity transition period, not confirmed which (if any) are still live in Baha Mar/Hyatt/MTS's contact records. Worth double-checking the address on file when booking this weekend's transportation, so the confirmation doesn't go somewhere unmonitored. |
...
- **Plaud Note Pro** — AI voice recorder; **✅ Purchased 8/13/26, ✅ subscribed 9/1/26.** Addresses the "lost breakthrough problem" — live client/training sessions previously relied on Pete's own after-the-fact Voice Memo brain dump, which can miss detail. Flagged 8/6/26 after seeing Bron Walis use one at Mohonk; researched 8/8–8/13/26 across new ($159 direct) vs. open box vs. used before settling on an open-box eBay unit. **Device order:** PN0300, 64GB, Black, "New – Open Box" condition, from seller "pennygold" (99.9% feedback, 1.2K sales, member since 2007) — eBay item #327307181434, order #27-14998-40150, placed & paid 8/13/26. **Total $140.16** ($124.99 item + $6.00 shipping + $9.17 tax), shipped USPS Ground Advantage to 13606 S 20th St, Bellevue NE. Seller confirmed the unit was previously account-bound but has already been unbound — Pete just creates a Plaud account and binds it fresh (Plaud app → Device page). **Subscription:** free trial 8/25–9/1/26 converted to the **Annual Unlimited Plan**, Sep 1 2026–Sep 1 2027, $215.99 paid 9/1/26 (10% first-payment discount off $239.99; renews at $239.99/year unless cancelled) — invoice 4IACSW5F-0002, receipt 2376-0127. **Both device and subscription logged as PWS business expenses** — see `finances/pws-business-expenses.md`; charged to FNBO Evergreen -0733; receipts archived in `OneDrive\Documents\Personal Finance\Receipts\2026\`.
  - **Workflow:** Plaud device records the live session → Plaud's app transcribes + diarizes speakers (labels who said what) automatically → exported `.txt` lands in `OneDrive\Transcriptions\` (the inbox) → Claude reads the raw transcript and writes a structured summary into the relevant client/project folder in this repo (action items, decisions, open issues — tailored to PWS/client context, not a generic AI summary) → raw transcript archived to `OneDrive\Transcriptions\Processed\` (never committed to the repo). Plaud's own built-in AI summary is largely redundant with this step and can generally be ignored once the repo summary exists.
  - **Whisper as a complementary (not competing) tool:** confirmed 9/9/26 that `faster-whisper` (free, local, open-source) can transcribe *existing* audio/video files — useful for archival material Plaud never recorded (e.g., the Mac McIntire video library, see `pws/mac-mcintire/mac-mcintire.md`). It does **not** do speaker diarization out of the box, so it's not a substitute for Plaud on live multi-speaker sessions — Plaud's hardware capture + diarization remain the right tool there.
...
- **Brother HL-L2380DW** — mono laser AIO; secondary receipt scanner for crumpled/fragile receipts; had dead power-on issue resolved July 2026 (unplugged power cord 60 sec, back in service). **BOLO replacement: Brother MFC-L2750DW** (adds 50-page ADF, duplex scan, fax, touchscreen, 36 ppm; same toner/drum ecosystem) — not urgent, buy on deal only: new target $200–220 (Black Friday/back-to-school), used $80–150. Pickup-only local listings can go much lower — reference: eBay item 137298404809 ended $33.90, 1 bid, "used sparingly," pickup only, Genuine toner installed (July 2026 — missed it). **Active saved eBay search running as of 8/20/26** — watching for a hit rather than manually re-searching. Backups: Brother DCP-L2640DW, HP LaserJet MFP M235sdw, Canon imageCLASS MF275dw. (Ruled out 8/20/26: Brother MFC-8460N — discontinued ~20 years, no wireless, no duplex scan.)
- **Document generation (Word/Excel/PDF from Claude Code) — established 9/18/26 building the BTTR/NPS invoice.** Reusable for any future formatted-document task (other invoices, letters, reports):
  - **Node.js + npm** — not preinstalled on this machine; installed via `winget install --id OpenJS.NodeJS.LTS` (publisher: OpenJS Foundation). Needed for Claude's built-in `docx` skill, which assumes Node.js is already present (true in Anthropic's cloud sandbox, not true on this local Windows PC) — install once, persists across sessions.
  - **`docx` npm package (docx-js)** — installed via `npm install docx` in the session scratchpad; builds `.docx` files programmatically (headers, tables, images, styled text) from a Node script. This is what Claude's `anthropic-skills:docx` skill uses under the hood to create new Word documents.
  - **Microsoft Word COM automation (via PowerShell)** — used to export a `.docx` to PDF (`$word.Documents.Open(...); $doc.ExportAsFixedFormat($outPath, 17)`). **This is the working PDF-export path on this machine** — the docx skill's own bundled `soffice.py` (LibreOffice-based) script failed here (`AttributeError: module 'socket' has no attribute 'AF_UNIX'` — it assumes a Linux/Mac environment) since LibreOffice isn't installed and the script itself isn't Windows-compatible. Word COM is the reliable substitute whenever the skill's own PDF-conversion step doesn't work locally.
  - **Microsoft Excel COM automation (via PowerShell)** — used to build a matching Excel version of the same invoice (cell styling, merged cells, embedded logo image, page setup/print area). Same general pattern as the Excel automation already used for expense reports.
  - **Python (3.12, already installed) + Pillow (PIL) + PyMuPDF (`pymupdf`/`fitz`)** — Pillow for sampling brand colors out of a logo image; PyMuPDF for rendering a PDF's pages to PNG images so Claude can actually look at the visual output and verify formatting before calling a document finished. `pymupdf` isn't preinstalled — installed via `pip install pymupdf` (fast, small package).
...
- **9/11/26: Pete texted Genevieve to prompt a reconnection** — this is already a carved-out direct-client property (Happy Hour Lessee LLC d/b/a MHBR), so re-engaging here doesn't carry the same contract-timing risk as WLP/Fabrizio. **9/18/26: followed up with a call, left a voicemail.** Still no response as of this entry.
...
- **✅ Ticket UNIFOCUS-259575 resolved** — Emre filed "Unable to upload KBI" 8/14/26 10:30am, marked urgent (risk was blocking his Monday 8/17 payroll budget start). Emre confirmed resolved via comment 8/21/26.
- **✅ Rosewood Butler standard built and tested 8/14/26** — 4→20 shift AM/PM ramp, 1→3 overnight, Lobby Host at 134 rooms, in Unifocus Standard Set "Budget Standards 2027." Tested live 8/14: 47.35 avg FTE vs. 44 target — close, minor tuning pending a department manager's return. **Emre starts his 2027 payroll budget Monday, August 17, 2026** using this standard (earlier/softer checkpoint than the previously tracked Wed 8/19 date). Guest Experience Associate job also built same session (5 shifts, PBX + Pre-Planning). Feeds Rosewood's first 2027 payroll budget submission (due end of August 2026). Full detail: `pws/clients/baha-mar/2026-08-14_friday-followup-call-butler-test-and-guest-experience.md`.
...
- **Makeready (part of Rockbridge)** — 3 properties total (Moran, Heathman, and Greenleigh — see below); Pete as trainer. Steve Carrell owns config; John Grech owns post-config; Taylor Walton owns post go-live follow-up.
  ...
  - **Heathman, Portland** — **Pushed, confirmed 9/10/26: personnel reasons (manager turnover at the property)** — consistent with the mid-manager-turnover rationale Ralph floated back on 7/14/26. Previously tentative for Oct 19–22, 2026; that date no longer holds — Pete's row on the current project resource calendar shows nothing for Heathman through year-end. No new date yet; watch for Alain/Steve Carrell to reschedule once the property's leadership settles.
...
- **HMAlpha (spin-off of Dimension)** — **Scope revised 7/17/26: confirmed 15 properties total** (DoubleTree Charlotte/Raleigh/San Diego Del Mar/Sunrise, Hilton Indianapolis/Scottsdale, Grand Hyatt Nashville, Hyatt Regency Valencia/Westlake, InterContinental New Orleans, Autograph Union Station Nashville, JW Marriott Houston, Marriott Del Mar/Warner Center, Westin La Paloma) — reverses the earlier "not 15" framing below; New Orleans + Nashville trained, WLP is #3 (Aug 17–21), 12 more still to schedule. Full contact sheet (GM/DOF/HR × 15) received from Nicole Mendez 7/17/26, added to contacts.csv. Pete as trainer. **Grand Hyatt Nashville confirmed 7/28/26: Sep 28–Oct 2, 2026** (see Upcoming Travel).
  - **DoubleTree Sunrise — reschedule proposed 8/13/26** (Ralph Varble email to Devon Peters/Nicole Mendez, cc Susanna Briggs, Alain Derderian, Steve Carrell, Akram Chahin, Pete): original plan was Steve configuring week of Sept 21, Akram training week of Oct 5. Ralph had no resource for Sept 21 and proposed pushing one week: Steve configures week of Sept 28, Pete trains week of Oct 12 (instead of Akram). **Confirmed via Ralph's resource calendar screenshot 8/14/26: Pete training DT Sunrise Oct 12–14, 2026 (Mon–Wed), no conflict with Heathman** (which sits the following week, Oct 19–22 — see Heathman entry below, date corrected from earlier Oct 12–15 estimate).
...
- **Unifocus team:** Ralph Varble (CCO) rvarble@unifocus.com O:972-512-5083 C:832-226-8945 — **⚠️ announced resignation 8/25/26, effective 8/31/26** (see Unifocus Contract Review memory for implications) | John Grech jgrech@unifocus.com | Akram Chahin achahin@unifocus.com | Taylor Walton twalton@unifocus.com | Susanna Briggs sbriggs@unifocus.com | Ahmed Chadid AChadid@unifocus.com
...
- **Aug 17–21, 2026** — Westin La Paloma Resort & Spa, Tucson AZ — HMAlpha onsite training (Indianapolis swapped out). **Outbound flight booked** (confirmed 8/13/26): Southwest BQR3JN, OMA-DEN-TUS, Sun 8/16, departs OMA 1:15 PM (WN1511) → DEN 1:50 PM → departs DEN 5:25 PM (WN4214) → TUS 6:25 PM. Seats 22A/26F. $528.20 (Visa ••••2674). **Tight turnaround note:** same-day as 11am checkout from the Denison, IA wedding hotel (Cobblestone Inn & Suites) — Denison to OMA is ~1.5hr drive, leaves little buffer before the 1:15 PM departure. **Return flight booked** (confirmed 8/13/26): Southwest A9QZXC, TUS-DEN-OMA, Fri 8/21, departs TUS 11:40 AM (WN2406) → DEN 2:30 PM → departs DEN 3:15 PM (WN4717) → OMA 5:40 PM. Seats 19A/09D (preferred). $453.20 (Visa ••••2674).
- **Aug 31, 2026 (week of)** — The Moran, Houston TX — Makeready onsite training — ✅ CONFIRMED (rescheduled 7/14/26 from original Aug 10–14 date). **Outbound flight booked** (confirmed 8/16/26): Southwest BUKKAA, OMA-HOU (Hobby), Sun 8/30, departs OMA 11:15 AM (WN4323), arrives HOU 1:30 PM. Seat 07A (Preferred). $254.40 (Visa ••••2674). **Return flight booked** (confirmed 8/16/26): Southwest BUGYTC, HOU (Hobby)-OMA, Fri 9/4, departs HOU 8:15 AM (WN3570), arrives OMA 10:25 AM. Seat 07A (Preferred). $365.40 (Visa ••••2674). **Once home Friday, Pete wants to leave some leeway for lunch with daughter Jennifer** (noted 9/3/26 evening) — keep the rest of Friday afternoon light when helping plan around this trip's tail end.
- **Sep 7, 2026 (week of)** — Marriott Del Mar, San Diego CA — HMAlpha onsite training — ✅ week confirmed 9/2/26 directly with Taylor Walton (configurer). Property contacts: Mark Ziomek (GM, engaged), Sandra Academia (DOF — actual key user who'll run the system), Valeria Ruvalcaba (HR, newer to system). **Outbound flight: Monday 9/7.** **Return flight changed (9/10/26):** originally Southwest #3440, confirmation **BEIJHG**, Fri 9/11 SAN→OMA nonstop 6:45 PM–11:50 PM — **now moved to Saturday 9/12** (same confirmation BEIJHG, new flight WN0734, departs SAN 12:15 PM arrives OMA 5:30 PM, seat 02F Extra Legroom, $57.00 change fee on Visa ••••2674, ticket #5262199565846). **Reason: Pete extended the San Diego trip a day to see his uncle on Friday 9/11** — a previously untracked San Diego-area family connection. DoubleTree San Diego Del Mar (same San Diego pair) has been reassigned to John Grech as trainer (config Oct 5, training Oct 19) — Pete no longer involved there, learned informally on this same call. Full detail: `pws/clients/hmalpha/marriott-del-mar/2026-09-02_taylor-walton-prep-call.md`.
- **Oct 12–14, 2026** — DoubleTree Sunrise — HMAlpha onsite training (per Ralph's resource calendar 8/14/26; replaces Akram as trainer, config done by Steve Carrell week of Sept 28)
- **Oct 19–22, 2026** — Heathman, Portland OR — Makeready onsite training — tentative (yellow on Ralph's consulting resource calendar as of 8/14/26; date corrected from earlier Oct 12–15 estimate)
- **Sep 20–26, 2026** — Disney Cruise, **Fort Lauderdale FL (Port Everglades)** — corrected 9/10/26; previously logged as Port Canaveral, but the actual outbound flight confirmation (Southwest CNZVLJ, forwarded by Brian Brazeal 7/7/26) flies the family into **FLL**, not MCO. **Personal** (Kim); blocked with Ralph Varble. Traveling with Jessica, Brian, Astrid, and Cecilia Brazeal. **Outbound:** Sat 9/19/26, WN3459/WN3612 (1 stop, plane change MDW), departs OMA 12:30 PM, arrives FLL 7:20 PM. Peter's ticket # 5262178068618, Kim's # 5262178068613. **Bahamas stop: Wednesday, Sep 23, 2026** — current plan has Pete back onboard by 5:30 PM. Ship is the **Disney Destiny**; a third-party port-schedule app (sent by Jessica) shows Nassau call times 8:00 AM–5:30 PM that day — matches Pete's 5:30 PM plan, but **treat as tentative** (unofficial source, not Disney's own itinerary). **Now also a business stop, confirmed 9/19/26: Pete is using this port day to visit Baha Mar** — lunch with Jon Finch, meeting Valquir Correa (confirmed) and hopefully Nicola (identity TBD), maybe Emre Kenan; will mention Jira ticket RMSOPS-12810 in person; deliberately asking Valquir/Nicola for an introduction before approaching the casino-prospecting contacts directly. See `pws/clients/baha-mar/2026-09-23_bahamas-cruise-stop-visit-plan.md`. **Return flight confirmed** (confirmation AZEVAR): Sat 9/26/26, WN4630/WN833 (1 stop, plane change BNA — i.e. Nashville), departs FLL 3:35 PM, arrives OMA 7:50 PM. Peter's ticket # 5262183280697. Lands Saturday evening with all of Sunday 9/27 free before the 2 PM NTSG picnic — resolves the Nashville-turnaround question, see `pws/clients/hmalpha/grand-hyatt-nashville/2026-09-10_logistics-arrangements-email.md`.
- **Sep 28–Oct 2, 2026** — Grand Hyatt Nashville — HMAlpha onsite training — ✅ CONFIRMED 7/28/26 (previously showed twice on Ralph's consulting resource calendar as tentative — weeks of Sept 28 and Oct 19 — now resolved to this single week). **Full 5-day training week (Mon–Fri) per the 9/10/26 project resource calendar** — not the shorter 4-day-plus-wrap pattern used at other properties. Sole onsite consultant, per Alain Derderian's 9/10/26 weekly-call recap. **Turnaround resolved:** Disney Cruise lands OMA Sat 9/26 7:50 PM; NTSG Annual Picnic Sun 9/27 2–4 PM, Papillion NE; then **outbound flight booked (confirmed 9/10/26):** Southwest confirmation CNTHN2, Sun 9/27, OMA-MDW-BNA (WN4717/WN0280), departs OMA 6:20 PM, arrives BNA 10:10 PM, seats 07F/09A (Preferred). $293.20 (Visa ••••2674). Ticket #5262199663949. **Return flight booked** (confirmed 9/14/26): Southwest confirmation BM7S76, Fri 10/2, BNA-MCO-OMA (WN3343/WN4804), departs BNA 4:35 PM, arrives MCO 7:30 PM, departs MCO 8:25 PM, arrives OMA 10:40 PM. Seats 27A/07F. $343.20 (Visa ••••2674). Ticket #5262400716397. See `pws/clients/hmalpha/grand-hyatt-nashville/2026-09-10_logistics-arrangements-email.md`.
...
- **Lewis & Clark Monuments** — reviving an original BTTR project, series of monuments
  ...
  - **NPS $2,450 Amplify Arts contribution — 9/18/26 send attempt bounced (SendAsDeniedException), needs resend.** NPS requires payment by cc/PayPal specifically (not check/EFT), due before end of September 2026. BTTR has no digital payment acceptance today — Pete evaluating FNBO/Clover vs. PayPal; Larry Foster has a fallback to route the payment through Papio-Missouri River NRD (P-MRNRD) if needed. See `BTTR/2026-09-18_nps-payment-routing-amplify-arts-funds.md` (send-failure detail) and `BTTR/2026-09-18_fnbo-payment-processing-consultation.md`.
...
- **BLSBD/standards-test tooling:** the budget-vs-labor-standards alignment check (BLSBD) is currently a fully manual, outside-the-software process — run the report, dump it into a spreadsheet, that spreadsheet is the deliverable. Pete flagged (9/17/26, Westin La Paloma call) that PWS will work on a better way to do this. Future idea, not yet scoped.
...
- [ ] **Surface Studio M365 auth broken (flagged 8/17/26, Westin La Paloma trip)** — Teams/Outlook throw `0x800704cf` "no internet" errors for the personal account (peter.a.castellano@outlook.com) and Outlook shows a "hasn't been activated" banner; Unifocus/work account still signs into Teams fine on the same device. Confirmed NOT a network/DNS/account-validity issue (ruled out on two networks, personal account works fine via browser and on the Lenovo Yoga). Tried and failed to fix: Teams app Reset, Token Broker Cache clear (`%LocalAppData%\Microsoft\TokenBroker\Cache`) + full reboot, Local Group Policy checked clean. Real impact: a Ralph Varble email and a client logistics email composed on the Surface never actually sent (confirmed absent from Sent Items) — **correction 8/18/26: those were actually resent from Pete's phone, not the Surface** — so the Surface's failure to send is a clean, total failure, not a selective/partial one. Microsoft ticket #7107414235 open. Next diagnostic not yet tried: new local Windows user profile test (isolates profile corruption vs. deeper OS/device issue). Pete brought both laptops on this trip (Westin La Paloma, Tucson) — Surface is with him, not left at home, and he normally uses the Surface specifically to present during training sessions, so this is a real risk for the rest of this week's HMAlpha training if it isn't resolved before the next session. **New development 8/18/26: the Lenovo Yoga is now ALSO failing to log into icloud.com, with the login screen blaming the network** — a similar symptom shape (network-blaming error) to the Surface's M365 failure, but on a different device and a different service (iCloud, not M365). This reopens the earlier "ruled out network" conclusion, which was based on the personal M365 account working fine on the Yoga at the time — that data point no longer holds now that the Yoga itself is showing a network-blaming login error. Not yet known whether this is the same root cause (shared hotel network/DNS/firewall issue at Westin La Paloma) or two unrelated device-specific problems — worth checking whether other devices/services on the same hotel network are affected before concluding either way. As of 8/18/26, M365 on the Yoga is still logged in and working fine — but given the Yoga's new iCloud failure, Pete is now concerned M365 access there could also drop, which would leave him with no working laptop for M365 (Surface already down) during this week's HMAlpha training at Westin La Paloma. **Diagnostic result 8/18/26 ~5:32pm: confirmed hotel-network issue, not device issue.** Switched Yoga from hotel wifi to Pete's iPhone hotspot — icloud.com loaded fine over the hotspot. This points at the Westin La Paloma guest network itself (likely SSL inspection/content filtering/DNS blocking auth-provider domains) as the actual root cause, not profile corruption on either laptop. Strongly suggests the Surface's M365 failure is the same root cause, just a different provider's auth endpoint being blocked — not yet confirmed by testing the Surface on hotspot. **Hotspot test on Surface, 8/18/26: mixed result — isolates the real problem.** On iPhone hotspot (hotel network fully bypassed), the Surface can log into iCloud AND Unifocus Teams fine, but the **personal M365 account still fails** with the identical `0x800704cf` "no internet" error (trace ID `02f8abfe-2b6c-0006-1207-47066c2bdd01`). Windows' own network tray icon also still shows "no internet" (globe icon) on the Surface even on hotspot with working connectivity otherwise. **Conclusion: this rules out both the hacked-account theory and the hotel-network theory for the Surface specifically** — a real hotel-network or OS-wide "no internet" problem would also break Unifocus Teams and iCloud, and it doesn't. The failure is isolated to (personal M365 account) × (Surface device), regardless of network — consistent with the original hypothesis: a broken Web Account Manager (WAM) / token cache / account registration for that one account on that one device, not network and not credentials. The hotel-network angle was still a real and useful finding for the **Yoga's iCloud problem** — just not the explanation for the Surface's M365 problem, which predates this week's network detour. **Next steps:** (1) try signing into the personal account via browser (outlook.office.com) on the Surface — if that works like iCloud did, it further confirms the native-app/WAM layer is the specific break point; (2) try fully removing and re-adding the personal Microsoft account under Settings > Accounts (more aggressive than the Token Broker Cache clear already tried); (3) the still-untried new local Windows profile test remains the cleanest way to confirm profile-level corruption; (4) feed the new trace ID to Microsoft support ticket #7107414235. Worth raising the filtering/SSL-inspection question with hotel IT per the earlier note, now with hotspot proof in hand rather than a guess. **8/18/26: Hotel IT's response was to suggest Pete may have been hacked and should change passwords for iCloud, M365 personal, and Unifocus.** Note: this doesn't fit the evidence — the hotspot test showed icloud.com working immediately on the same device/same credentials just by switching networks, which is a network-layer symptom (SSL inspection/DNS/firewall breaking auth traffic), not what a compromised-credential failure looks like (that would fail the same way regardless of network). If Pete does rotate passwords as a precaution, do it over hotspot/cellular, not hotel wifi — if the hotel network is doing SSL inspection, typing new passwords through that same connection risks exposing them too. Re-testing Surface via hotspot again 8/18/26 (first hotspot test on Surface was yesterday, 8/17). **Correction 8/18/26: the "Next steps" browser test wasn't run yet at that point** — the `...ef7d-4a066c2bdd01` trace ID was actually from launching the **Outlook desktop app** on the Surface (still on hotspot), failing before the account picker even appeared, not from the browser. **The actual browser test (outlook.office.com on the Surface, on hotspot) was run afterward and also failed.** So on the Surface via hotspot: iCloud ✅, Unifocus Teams ✅ (likely a cached/already-authenticated session, not a fresh login), personal M365 via Outlook app ❌, personal M365 via browser ❌. Both app and browser failing points to something specific to Microsoft's login/identity domains (login.microsoftonline.com / login.live.com) breaking on the Surface itself — independent of network, and not explained by a compromised account. Pete is photographing/screenshotting the Surface's browser error to attach here; screenshot pending as of this entry (written from the Yoga). **Screenshot received 8/18/26: the outlook.office.com browser failure's actual error text is `AADSTS500200`** — "Personal Microsoft accounts are not supported for this application unless explicitly invited to an organization." That specific portal is org-only by design, so this alone isn't new breakage, but it's a real clue: worth asking whether the native Outlook/Teams apps on this Surface are silently routing the personal account's sign-in through an organization-scoped path (plausibly tied to the Unifocus account being the primary/default identity on this machine) and hitting the same rejection, just displayed as the generic `0x800704cf` error instead of the real AADSTS message. **Also worth raising directly with Unifocus IT**, not just Microsoft support, since a tenant-side Conditional Access/restriction policy would explain this cleanly. **Further escalation 8/18/26, evening:** the Unifocus account — previously reliable everywhere — also started failing in native Outlook desktop: clicking "Needs Password" never reaches a password prompt, throws the same `0x800704cf` error instead. Native Outlook and Teams desktop are now unreliable for *both* accounts on the Surface. Confirmed working fallback: `outlook.office.com` in browser for the Unifocus account, `outlook.com`/`teams.microsoft.com` in browser for the personal account — both training sessions on 8/18 (9am, 2pm) ran fine regardless, since presenting relies on the browser-based Unifocus P&S system, not the broken native apps. Separately, the **Lenovo Yoga** also hit an unrelated Windows Hello hiccup on 8/18 ("something went wrong and your pin isn't available," face recognition failed) — cleared with a simple reboot; distinct symptom from the Yoga's iCloud/hotel-network issue above, likely just a normal post-sleep glitch. Pete stopped active troubleshooting the evening of 8/18 after two full training sessions on top of all this.
- [ ] **Email sent from Excel/Office apps can silently use the wrong mail identity — watch for this.** Flagged 9/3/26: a training-timeline email sent to Monica Brock (The Moran) directly from inside Excel did not appear in Pete's Unifocus Sent Items, and its sender DN read `/O=FIRST ORGANIZATION/...` instead of the `/O=EXCHANGELABS/...` pattern every confirmed Unifocus send shows — strong signal it went from a personal account instead of pcastellano@unifocus.com, with no way to confirm which one (no connector access to personal accounts). Distinct from the Surface/Yoga M365 login-failure issues below — this is a *silent misdirect*, not a visible error. When sending an attachment straight from an Office app (not composing in Outlook directly), verify the From/account afterward, especially on client-facing correspondence that other Unifocus staff are cc'd on and expect to find in the shared thread.
...
- [ ] **Baha Mar Casino** — followed up with Clay Price 9/2/26 (email, awaiting reply); **Ed Peek has left Unifocus** (learned via Ahmed Chadid, 9/2/26) — new sales contact is **Alex Seltzer** (not yet reached). Ahmed also mentioned a CSM meet-and-greet Friday 9/4/26, 2 PM ET (for the existing CSM team, not Pete). Unifocus reportedly closing a deal; Pete likely doing the implementation. **Pete's fallback (noted 9/2/26): if Unifocus can't land this add-on, Pete will pursue the casino ops work himself directly.** Carve-out confirmed clear: CTF BM Operations d/b/a Baha Mar (the existing carve-out) is the owning entity that would decide to add casino to Unifocus — it owns Rosewood, SLS, Grand Hyatt, and the CTF Operations property record itself. Pete already has separate direct agreements with Rosewood and Grand Hyatt (CTF-approved) and is onboarded as a vendor with both. See `pws/clients/baha-mar/2026-09-02_casino-prospecting-followup-email.md`.
...
- [x] **Calendar coordination** — Full consulting schedule confirmed via Ralph's resource calendar screenshot 8/14/26: Aug 2–8 (Mohonk), Aug 17–21 (Westin La Paloma Tucson), Aug 31 week (Moran Houston), Sep 7 (Marriott Del Mar — confirm), Sep 28–Oct 2 (Grand Hyatt Nashville), Oct 12–14 (DT Sunrise), Oct 19–22 (Heathman Portland — still tentative/yellow), Greenleigh still needs approximate Q4 dates. Personal blocks: Sep 20–26 Disney, Nov 27–Dec 6 MSC. DT Sunrise/Heathman conflict flagged 8/13/26 is resolved — separate weeks, no overlap.
...
- [ ] **At Bellevue home office: run Brother firmware updater** — `BrMain4905.exe` in Downloads (Jun 7 + Aug 31 copies, same build) is a self-extracting Brother Firmware Update Tool, for either the ADS-2000 scanner or HL-L2380DW printer (both live at the home office). Run it once at the machine physically connected to the device.
